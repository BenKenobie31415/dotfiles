...
ignored_window_names: list[str] = ["Eww - bar_main", "Eww - darkbar_main"]
...
def update_focused_window_name() -> None:
    update_eww_var("focusedname", get_cropped_window_name(bspc.get_focused_node()))

def update_eww_var(var: str, value: str) -> None:
    os.popen(f"eww update {var}={value}")

def get_cropped_window_name(window_id: str) -> str:
    name: str = bspc.get_node_name(window_id)
    print(name)
    if len(name) <= 64:
        return name
    name = name[0:61] + "..."
